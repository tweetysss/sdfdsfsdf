...
let rol = db.fetch(`anan12_${message.guild.id}`)  
  if (!rol) return message.channel.send(`:white_small_square: Bot Ekleme Log Kanalı Yok!`)
  message.channel.send(`:white_small_square: BOT Ekleme Log Sildim reisim`)
...
const embed = new Discord.MessageEmbed()
.setColor('BLUE')
.setDescription(`:white_small_square: **Bot Ekleme Log kanalı <#${kinal}> Kanalına Ayarlı! \n Kapatmak için Lütfen ** \`${ayarlar.prefix}bot-ekleme-log sıfırla Yazınız. \``)
return message.channel.send(embed)
}
